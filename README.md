...
这个仓库只保存可公开复用的规则、模板和路由约定。个人背景、真实简历、公司流程、面试记录、逐字稿和复盘结果保存在用户选定的唯一私有知识库中。
...
私有知识库可以使用本地 Codex 项目、Feishu/Lark 或其他已授权的文档工具，但同一求职工作流只维护一个 Hub。skill 不保存固定账号、文档链接、空间 ID 或访问凭证。
